...
const TAG_RULES = [
  ...
  },
  {
    ...
    tags: ['skincare-science'],
  },
  {
    keywords: ['猫粮', '狗粮', '猫砂', '猫咪', '狗狗', '宠物', '驱虫', '疫苗'],
    tags: ['pet-nutrition'],
  },
  {
    keywords: [
...
function generateSlug(filename) {
  ...
  let zhTitle = numMatch[2].replace(/\.md$/, '').replace(/_/g, ' ')
...
async function main() {
  ...
  for (let i = 0; i < articles.length; i += BATCH_SIZE) {
    ...
    const { error } = await supabase.from('pitfallfree_guides').upsert(
