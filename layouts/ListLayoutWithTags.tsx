...
export default function ListLayoutWithTags({
  ...
  // Extract locale from pathname: /en/... or /zh/...
  const localeMatch = pathname.match(/^\/(en|zh)\//)
  const locale = localeMatch ? localeMatch[1] : 'en'

  // Build tag counts from locale-specific posts
  const tagCounts: Record<string, number> = {}
  allBlogs.forEach((post) => {
    const postLocale = (post as { locale?: string }).locale || 'en'
    if (postLocale === locale && !post.draft && post.tags) {
  ...
  const isOnBlogRoot = pathname === blogPath || pathname === `${blogPath}/`

  const allPostsLabel = locale === 'zh' ? '全部指南' : 'All Guides'
        ...
        <div className="flex sm:space-x-24">
          <div className="hidden h-full max-h-screen max-w-[280px] min-w-[280px] flex-wrap overflow-auto rounded-sm bg-gray-50 pt-5 shadow-md sm:flex dark:bg-gray-900/70 dark:shadow-gray-800/40">
            <div className="px-6 py-4">
              {isOnBlogRoot ? (
                <h3 className="text-primary-500 font-bold uppercase">{allPostsLabel}</h3>
              ) : (
                <Link
                  href={blogPath}
